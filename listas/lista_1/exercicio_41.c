/*
 * 41. Escreva uma função que tenha como parâmetros ponteiros para duas listas. A função deve retornar
 * um ponteiro para uma terceira lista, que é o resultado da união das duas (isto é, os elementos repetidos
 * devem ser considerados apenas uma vez).
 *
 */
...
Lista *uniao_de_lista(Lista *lista1, Lista *lista2){
  ...
  while(lista1 != NULL){
    ...
    if(!existe(uniao, valor)){
      uniao = inserir_no_final(uniao, valor);
    }
    lista1 = lista1->prox;
  }
  while(lista2 != NULL){
    int valor = lista2->num;
    if(!existe(uniao, valor)){
      uniao = inserir_no_final(uniao, valor);
    }
    lista2 = lista2->prox;
...
int main(){
  ...
  printf("41. Escreva uma função que tenha como parâmetros ponteiros para duas listas. A função deve retornar"
         "um ponteiro para uma terceira lista, que é o resultado da união das duas (isto é, os elementos repetidos"
         "devem ser considerados apenas uma vez).\n\n");
  printf("\tlista1 -> ");
  imprimir(lista1);
